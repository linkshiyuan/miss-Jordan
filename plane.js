...
function starGame() {
  ...
  // 找到子弹
  findBulletTime = setInterval(findBullet, 120);
  ...
  // 发射子弹
  BulletFly = setInterval(bulletFly, 15);
  function bulletFly() {
    for (let j = 0; j < bullets.length; j++) {
      if (bullets[j].offsetTop >= 30) {
        bullets[j].style.top = bullets[j].offsetTop - 8 + "px";
      } else {
        bullets[j].style.top = "-50px";
        bullets[j].style.left = "-50px";
      }
    }
  }
  ...
  //飞机碰到敌人
  function hitPlane(enemys) {
    for (let enemy of enemys) {
      if (checkIntersect(enemy, plane) && !plane.querySelector(".boom")) {
        let boomm = document.createElement("div");
        boomm.classList.add("boom");
        boomm.style.transform = "scale(2)";
        boomm.style.top = "40px";
        boomm.style.left = "20px";
        plane.append(boomm);
  ...
  setInterval(hitPlane, 50, enemys1);
  setInterval(hitPlane, 50, enemys2);
}
...
function recoverPlane(plane) {
  ...
  let recordBg = document.querySelector(".record");
